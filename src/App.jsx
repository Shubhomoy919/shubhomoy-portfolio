     ...
     {/* --- 2. ENGINEERING MANIFESTO --- */}
      ...
      <section id="experience" className="relative z-10 border-t border-slate-900 bg-[#030508]/40 backdrop-blur-sm py-24">
        <RevealSection>
          <div className="max-w-5xl mx-auto px-6">
            <h2 className="text-5xl md:text-6xl font-extrabold mb-20 tracking-tight text-white flex items-center gap-5">
              <span className="w-10 h-1 bg-indigo-500"></span>
              Education & Experience
            </h2>
            
            <div className="relative border-l-2 border-slate-800 ml-4 md:ml-6 space-y-20">
              {trajectory.map((item, index) => (
                <div key={index} className="relative pl-10 md:pl-14 group">
                  <div className={`absolute -left-[9px] top-1.5 h-4 w-4 rounded-full bg-[#030508] border-2 ${item.color} group-hover:scale-150 group-hover:bg-current transition-all duration-300`}></div>
                  
                  <div className="flex flex-col md:flex-row md:items-baseline gap-3 md:gap-5 mb-3">
                    <h3 className="text-3xl md:text-4xl font-bold text-white tracking-tight">{item.role}</h3>
                    <span className="text-slate-500 font-mono text-lg tracking-widest uppercase">{item.timeline}</span>
                  </div>
                  
                  <h4 className="text-2xl text-indigo-400 font-medium mb-5">
                    {item.entity} 
                    <span className="text-slate-600 ml-3 text-lg uppercase tracking-widest font-mono">[{item.type}]</span>
                  </h4>
                  
                  <p className="text-slate-300 text-lg md:text-xl font-light leading-relaxed max-w-3xl mb-8">
                    {item.desc}
                  </p>
                    ...
                    <div className="flex flex-wrap items-center gap-5 pt-3">
                      ...
                      {item.lorLink && (
                        <a 
                          ...
                          target="_blank" 
                          rel="noreferrer"
                          className="group/lor relative inline-flex items-center gap-2 px-6 py-3 rounded-xl text-base font-mono font-medium text-emerald-300 bg-emerald-950/40 border border-emerald-500/30 hover:border-emerald-400 hover:bg-emerald-500/20 hover:text-white transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.25)] hover:-translate-y-0.5 active:scale-95 overflow-hidden"
                        >
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-10 hover:border-slate-700 transition-all duration-300 flex flex-col relative overflow-hidden group shadow-xl">
                ...
                <p className="text-slate-500 font-mono text-sm tracking-widest uppercase mb-8 flex items-center gap-3 relative z-10">
      ...
      {activeProject && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8">
          ...
          <div className="relative w-full max-w-6xl bg-[#030508] border border-slate-700 rounded-3xl shadow-[0_0_100px_rgba(0,0,0,0.95)] overflow-hidden flex flex-col max-h-[90vh] ring-1 ring-white/10">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center px-8 py-6 border-b border-slate-800 bg-[#070b14]">
              <div className="flex items-center gap-5 mb-5 md:mb-0">
                <span className="text-amber-500 font-mono text-base tracking-widest uppercase border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 rounded-lg">
                  {activeProject.id}.exe
                </span>
                <h3 className="text-white font-bold tracking-tight text-2xl">{activeProject.title} // Command Center</h3>
              </div>
              
              <div className="flex items-center gap-3 bg-slate-900 rounded-xl p-2 border border-slate-800">
                <button 
                  onClick={() => setModalTab("architecture")}
                  className={`px-6 py-3 text-base font-mono tracking-widest uppercase rounded-lg transition-all ${modalTab === 'architecture' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  Architecture
                </button>
                <button 
                  onClick={() => setModalTab("telemetry")}
                  className={`px-6 py-3 text-base font-mono tracking-widest uppercase rounded-lg transition-all ${modalTab === 'telemetry' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  Telemetry
                </button>
                ...
                <button 
                  onClick={() => setActiveProject(null)}
                  className="px-6 py-3 text-base font-mono tracking-widest uppercase text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"
                >
                  Close
                </button>
              </div>
            ...
            <div className="p-8 md:p-12 overflow-y-auto relative">
              ...
              <div className="relative z-10 flex flex-col gap-10">
                {modalTab === 'architecture' ? (
                  <div className="w-full bg-[#070b14] border border-slate-800 rounded-3xl p-10 relative overflow-hidden">
                    ...
                    <h4 className="text-slate-400 font-mono text-base uppercase tracking-widest mb-12 relative z-10 flex items-center gap-4">
                    ...
                    <div className="relative z-10 pl-6 md:pl-10 border-l-2 border-slate-800 space-y-12 py-3">
                      {activeProject.blueprint.map((step, index) => (
                        <div key={index} className="relative pl-8 md:pl-12">
                          <div className={`absolute -left-[1.1rem] md:-left-[1.35rem] top-2 w-4 h-4 md:w-6 md:h-6 rounded-full bg-slate-950 border-4 ${
                          ...
                          {index !== activeProject.blueprint.length - 1 && (
                            <div className="absolute -left-6 md:-left-7 top-16 text-slate-600 text-2xl">↓</div>
                          )}

                          <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-6 hover:border-slate-600 transition-colors">
                            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3">
                              <h5 className="text-white font-bold text-lg md:text-xl">{step.node}</h5>
                              <span className={`font-mono text-sm px-4 py-2 rounded-lg bg-slate-950 border border-slate-800 ${
                            ...
                            <p className="text-slate-400 text-base md:text-lg">{step.desc}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="grid grid-cols-2 gap-8">
                      {Object.entries(activeProject.metrics).map(([key, value], i) => (
                        <div key={i} className="bg-[#070b14] border border-slate-800 rounded-3xl p-8 hover:border-slate-600 transition-colors">
                          <p className="text-slate-500 font-mono text-xs uppercase tracking-widest mb-4">{key}</p>
                          <p className="text-white font-mono text-lg md:text-2xl">{value}</p>
                        </div>
                      ))}
                    </div>

                    <div className="bg-[#070b14] border border-slate-800 rounded-3xl p-10 font-mono text-base h-full flex flex-col min-h-96">
                      <div className="flex justify-between items-center border-b border-slate-800 pb-5 mb-6">
                        ...
                        <p className="text-emerald-500 animate-pulse text-sm">● REC</p>
                      </div>
                      
                      <div className="grow space-y-5">
                        {activeProject.logs.map((log, i) => (
                          <div key={i} className="flex flex-col sm:flex-row sm:gap-5 opacity-90 hover:opacity-100 transition-opacity">
                            <span className="text-slate-600 shrink-0">[{log.time}]</span>
                            <div className="flex gap-4">
                              <span className={`shrink-0 w-24 ${log.color}`}>[{log.level}]</span>
                ...
                <div className="pt-10 mt-5 border-t border-slate-800/80 flex flex-col sm:flex-row gap-6 justify-end">
                  <a href={activeProject.github} target="_blank" rel="noreferrer" className="px-12 py-5 rounded-2xl border border-slate-700 text-slate-300 font-mono text-base tracking-widest uppercase hover:bg-slate-800 hover:text-white transition-all text-center flex items-center justify-center gap-4">
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" /></svg>
                    Source Code
                  </a>
                  <a href={activeProject.live} target="_blank" rel="noreferrer" className="px-12 py-5 rounded-2xl bg-amber-500 text-slate-950 font-black tracking-widest text-base uppercase hover:bg-amber-400 hover:shadow-[0_0_30px_rgba(245,158,11,0.5)] transition-all text-center flex items-center justify-center gap-3">
                    Execute Live ↗
                  </a>
                </div>
              </div>
